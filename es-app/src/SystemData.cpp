...
SystemData::SystemData(
    const SystemMetadata &meta,
    SystemEnvironmentData *envData,
    std::vector<EmulatorData> *pEmulators,
    bool CollectionSystem,
    bool groupedSystem,
    bool withTheme,
    bool loadThemeOnlyIfElements) :
    mMetadata(meta),
    mEnvData(envData), mIsCollectionSystem(CollectionSystem), mIsGameSystem(true) {
    ...
    mHidden =
        (mIsCollectionSystem
             ? withTheme
             : (std::find(hiddenSystems.cbegin(), hiddenSystems.cend(), getName()) != hiddenSystems.cend()));
    ...
    if (withTheme &&
        (!loadThemeOnlyIfElements || UIModeController::LoadEmptySystems() || mRootFolder->mChildren.size() > 0)) {
        loadTheme();
...
void SystemData::populateFolder(FolderData *folder, std::unordered_map<std::string, FileData *> &fileMap) {
    ...
    if (Utils::FileSystem::isSymlink(folderPath))
    {
        ...
        {
            LOG_S(WARNING) << "Skipping infinitely recursive symlink \"" << folderPath << "\"";
            return;
        }
    }
    */
    ...
    for (auto fileInfo : dirContent) {
        ...
        if (!isGame && fileInfo.directory) {
            ...
            if (preloadMedias && (!mHidden || Settings::HiddenSystemsShowGames())) {
                ...
                if (fn == "manuals" || fn == "images" || fn == "videos" ||
                    Utils::String::startsWith(fn, "downloaded_")) {
            ...
            if (fn == "media" || fn == "medias" || fn == "images" || fn == "manuals" || fn == "videos" ||
                fn == "assets" || Utils::String::startsWith(fn, "downloaded_") || Utils::String::startsWith(fn, "."))
                continue;
...
void SystemData::createGroupedSystems() {
    ...
    for (auto sys : sSystemVector) {
        ...
        if (Settings::getInstance()->getBool(sys->getSystemEnvData()->mGroup + ".ungroup") ||
            Settings::getInstance()->getBool(sys->getName() + ".ungroup"))
            continue;
    ...
    for (auto item : map) {
        ...
        if (root->getChildren().size() > 0 && !existingSystem) {
            ...
            auto gridSizeOverride =
                Vector2f::parseString(Settings::getInstance()->getString(system->getName() + ".gridSize"));
        ...
        if (it != CustomFeatures::EmulatorFeatures.cend()) {
            ...
            for (auto &core : emul.cores) {
                for (auto escore : it->second.cores) {
                    ...
                    core.netplay =
                        (core.features & EmulatorFeatures::Features::netplay) == EmulatorFeatures::Features::netplay;
                }
            }
        }
    }
...
bool SystemData::isFeatureSupported(
    std::string emulatorName, std::string coreName, EmulatorFeatures::Features feature) {
    if (emulatorName.empty() || emulatorName == "auto")
        emulatorName = getEmulator();
...
void SystemData::loadAdditionnalConfig(pugi::xml_node &srcSystems) {
    ...
    for (auto rootPath : VectorHelper::distinct(rootPaths, [](auto x) { return x; })) {
        for (auto customPath : Utils::FileSystem::getDirContent(rootPath, false, false)) {
            ...
            if (!systemList) {
                LOG_S(ERROR) << Utils::FileSystem::getFileName(customPath) << " is missing the <systemList> tag !";
                return;
            }
...
bool SystemData::loadConfig(Window *window) {
    ...
    LOG_S(INFO) << "Loading system config file " << path << "...";

    if (!Utils::FileSystem::exists(path)) {
        LOG_S(ERROR) << "es_systems.cfg file does not exist!";
        return false;
    }
    ...
    if (!res) {
        LOG_S(ERROR) << "Could not parse es_systems.cfg file!";
    ...
    pugi::xml_node systemList = doc.child("systemList");
    if (!systemList) {
        LOG_S(ERROR) << "es_systems.cfg is missing the <systemList> tag!";
        return false;
    }
    ...
    if (systemCount == 0) {
        LOG_S(ERROR) << "no system found in es_systems.cfg";
        return false;
    }
        ...
        if (pThreadPool != NULL) {
            ...
            if (window != NULL)
                window->renderSplashScreen(
                    fullname, systemCount == 0 ? 0 : (float)currentSystem / (float)(systemCount + 1));
    ...
    if (pThreadPool != NULL) {
        if (window != NULL) {
            pThreadPool->wait(
                [window, &processedSystem, systemCount, &systemsNames] {
...
SystemData *SystemData::loadSystem(pugi::xml_node system, bool fullMode) {
    ...
    for (const auto &platformStr : platformStrs) {
        const char *str = platformStr.c_str();
        ...
        if (platformId != PlatformIds::PLATFORM_UNKNOWN)
            ...
            LOG_S(WARNING) << "  Unknown platform for system \"" << md.name << "\" (platform \"" << str
                            << "\" from list \"" << platformList << "\")";
    }
    ...
    if (path[0] != '/' && path[0] != '\\')
    {
        path = Utils::FileSystem::getCanonicalPath(Paths::getExePath() + "/config/systems/" + path);
    }

    // validate
    if (fullMode && (md.name.empty() || path.empty() || extensions.empty() || cmd.empty())) {
        LOG_S(ERROR) << "System \"" << md.name << "\" is missing name, extension, or command!";
        return nullptr;
    }

    if (fullMode && !UIModeController::LoadEmptySystems() && !Utils::FileSystem::exists(path)) {
        LOG_S(ERROR) << "System \"" << md.name << "\" path does not exist !";
        return nullptr;
    }
    ...
    if (emulatorsNode != nullptr) {
        for (pugi::xml_node emuNode = emulatorsNode.child("emulator"); emuNode;
             emuNode = emuNode.next_sibling("emulator")) {
            ...
            if (emuNode.attribute("incompatible_extensions")) {
                for (auto ext : readList(emuNode.attribute("incompatible_extensions").value())) {
                    std::string extlow = Utils::String::toLower(ext);
                    if (std::find(
                            emulatorData.incompatibleExtensions.cbegin(),
                            emulatorData.incompatibleExtensions.cend(),
                            extlow) == emulatorData.incompatibleExtensions.cend())
            ...
            if (coresNode != nullptr) {
                for (pugi::xml_node coreNode = coresNode.child("core"); coreNode;
                     coreNode = coreNode.next_sibling("core")) {
                    CoreData core;
                    core.name = coreNode.text().as_string();
                    core.netplay =
                        coreNode.attribute("netplay") && strcmp(coreNode.attribute("netplay").value(), "true") == 0;
                    core.isDefault =
                        coreNode.attribute("default") && strcmp(coreNode.attribute("default").value(), "true") == 0;

                    if (coreNode.attribute("incompatible_extensions")) {
                        for (auto ext : readList(coreNode.attribute("incompatible_extensions").value())) {
                            std::string extlow = Utils::String::toLower(ext);
                            if (std::find(
                                    core.incompatibleExtensions.cbegin(), core.incompatibleExtensions.cend(), extlow) ==
                                core.incompatibleExtensions.cend())
    ...
    if (!UIModeController::LoadEmptySystems() && newSys->getRootFolder()->getChildren().empty()) {
        LOG_S(WARNING) << "System \"" << md.name << "\" has no games! Ignoring it.";
        delete newSys;
        return nullptr;
    }
...
std::string SystemData::getConfigPath() {
    std::string exePath = Paths::getEmulationStationPath() + "/config/es_systems.cfg";
...
bool SystemData::isVisible() {
    if (mIsCollectionSystem) {
        if (mMetadata.name != "favorites" && !UIModeController::getInstance()->isUIModeFull() &&
            getGameCountInfo()->totalGames == 0)
            return false;
    ...
    if (!mHidden && !mIsCollectionSystem &&
        (UIModeController::LoadEmptySystems() || getGameCountInfo()->totalGames > 0))
        return true;
...
void SystemData::loadTheme() {
    ...
    try {
        ...
        sysData["global.cheevos.username"] = SystemConf::getInstance()->getBool("global.retroachievements")
        ...
        sysData["global.netplay.username"] = SystemConf::getInstance()->getBool("global.netplay")
                                                 ? SystemConf::getInstance()->get("global.netplay.nickname")
                                                 : "";
        ...
        // Retrocompatibility
        sysData["cheevos.username"] = SystemConf::getInstance()->getBool("global.retroachievements")
                                          ? SystemConf::getInstance()->get("global.retroachievements.username")
                                          : "";

        // System variables
        sysData["system.cheevos"] =
            SystemConf::getInstance()->getBool("global.retroachievements") && (isCheevosSupported() || isCollection())
                ? "true"
                : "false";
        sysData["system.netplay"] =
            SystemConf::getInstance()->getBool("global.netplay") && isNetplaySupported() ? "true" : "false";
...
bool SystemData::setSystemViewMode(std::string newViewMode, Vector2f gridSizeOverride, bool setChanged) {
    ...
    if (setChanged) {
        ...
        Settings::getInstance()->setString(
            getName() + ".gridSize",
            Utils::String::replace(Utils::String::replace(mGridSizeOverride.toString(), ".000000", ""), "0 0", ""));
    }
...
bool SystemData::isNetplaySupported() {
    ...
    if (!CustomFeatures::FeaturesLoaded)
        return getSystemEnvData() != nullptr &&
               getSystemEnvData()->mLaunchCommand.find("%NETPLAY%") != std::string::npos;
    ...
    if (mIsCheevosSupported < 0) {
        ...
        if (!CustomFeatures::FeaturesLoaded) {
            const std::set<std::string> cheevosSystems = {
                "megadrive",  "n64",        "snes",          "gb",        "gba",        "gbc",
                "nes",        "fds",        "pcengine",      "segacd",    "sega32x",    "mastersystem",
                "atarilynx",  "lynx",       "ngp",           "gamegear",  "pokemini",   "atari2600",
                "fbneo",      "fbn",        "virtualboy",    "pcfx",      "tg16",       "famicom",
                "msx1",       "psx",        "sg-1000",       "sg1000",    "coleco",     "colecovision",
                "atari7800",  "wonderswan", "pc88",          "saturn",    "3do",        "apple2",
                "neogeo",     "arcade",     "mame",          "nds",       "arcade",     "megadrive-japan",
                "pcenginecd", "supergrafx", "supervision",   "snes-msu1", "amstradcpc", "dreamcast",
                "psp",        "jaguar",     "intellivision", "vectrex",   "megaduck",   "arduboy",
                "wasm4"};
...
bool SystemData::shouldExtractHashesFromArchives() {
    return !hasPlatformId(PlatformIds::ARCADE) && !hasPlatformId(PlatformIds::NEOGEO) &&
           !hasPlatformId(PlatformIds::DAPHNE) && !hasPlatformId(PlatformIds::LUTRO) &&
           !hasPlatformId(PlatformIds::SEGA_DREAMCAST) && !hasPlatformId(PlatformIds::ATOMISWAVE) &&
