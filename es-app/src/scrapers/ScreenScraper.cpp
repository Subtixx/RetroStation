...
void ScreenScraperRequest::processGame(const pugi::xml_document& xmldoc, std::vector<ScraperSearchResult>& out_results)
{
	...
	for (pugi::xml_node game = data.child("jeu"); game; game = game.next_sibling("jeu"))
	{
		...
		if (_date.length() > 4)
			result.mdl.set(MetaDataId::ReleaseDate, std::string (Utils::DateTime(Utils::Time::stringToTime(_date, "%Y-%m-%d"))));
		else if (_date.length() > 0)
			result.mdl.set(MetaDataId::ReleaseDate, std::string (Utils::DateTime(Utils::Time::stringToTime(_date, "%Y"))));
