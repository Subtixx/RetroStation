...
#define BUFFER_SIZE 8192
    ...
    DWORD dwLen =
        ExpandEnvironmentStringsW(Utils::String::convertToWideString(command).c_str(), szEnvPath, BUFFER_SIZE);
    ...
    if (!Utils::String::startsWith(exe, ".") && !Utils::String::startsWith(exe, "/") &&
        ...
        wpath =
            Utils::String::convertToWideString(Utils::FileSystem::getAbsolutePath(Utils::FileSystem::getParent(exe)));
...
void processQuitMode() {
    ...
    case QuitMode::RESTART:
        LOG_S(INFO) << "Restarting EmulationStation";
        touch("/tmp/restart.please");
        break;
    case QuitMode::REBOOT:
    case QuitMode::FAST_REBOOT:
        LOG_S(INFO) << "Rebooting system";
        touch("/tmp/reboot.please");
        runRestartCommand();
        break;
    case QuitMode::SHUTDOWN:
    case QuitMode::FAST_SHUTDOWN:
        LOG_S(INFO) << "Shutting system down";
...
std::string QueryIpAddress() {
    ...
    for (ifa = ifAddrStruct; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        if (ifa->ifa_addr->sa_family == AF_INET) {
            ...
            std::string const ifName = ifa->ifa_name;
