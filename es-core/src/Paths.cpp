...
Paths::Paths() {
    ...
    loadCustomConfiguration(
        false); // Try to detect alternate paths ( Decorations, Shaders... ) without loading overrides
    ...
    /* EmuElec sample locations.
        ...
        mUserEmulationStationPath = Utils::FileSystem::getCanonicalPath(Utils::FileSystem::getHomePath() +
    "/.emulationstation"); mLogPath = "/storage/.config/emuelec/logs"; mThemesPath = mEmulationStationPath + "/themes";
            ...
            if (variable == "kodi") {
                if (Utils::FileSystem::exists("C:\\Program Files\\Kodi\\kodi.exe"))
                    ...
                    ret[variable] = "C:\\Program Files (x86)\\Kodi\\kodi.exe";
                             ...
                             Utils::FileSystem::combine(Utils::FileSystem::getParent(relativeTo), "kodi\\kodi.exe")))
                    ret[variable] =
                        Utils::FileSystem::combine(Utils::FileSystem::getParent(relativeTo), "kodi\\kodi.exe");
...
std::string &Paths::getHomePath() {
    ...
    // On Windows,  "HOME" is not a system variable but a user's environment variable that can be defined by users in
    // batch files. If defined : The environment variable has priority over all
    char *envHome = getenv("HOME");
    if (envHome != nullptr) {
        homePath = Utils::FileSystem::getGenericPath(envHome);
        if (Utils::FileSystem::exists(homePath + "/.emulationstation")) {
            return homePath;
        }
    }
    ...
    homePath = Utils::FileSystem::getGenericPath(Utils::FileSystem::getCWDPath());
